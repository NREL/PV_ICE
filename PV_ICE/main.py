                    ...
                    if x[age] == 0.0:
                        ...
                        if age == int(row['mod_lifetime']+generation):
                            ...
                            if deg_nameplate > 0.8:
                                ...
                                area_resold0 = area_collected*(df.iloc[age]['mod_EOL_pg2_resell']*0.01)
...
#                            disposed_projectlifetime = activearea_temp-activearea
            ...
            Matrix_Landfilled_noncollected = Matrix_Landfilled_noncollected.add_prefix("EOL_L1_Year_")
            

            try:
                df = df[df.columns.drop(list(df.filter(regex='EOL_L1_Year_')))]
            except:
                print("Warning: Issue dropping EOL columns generated by " \
                      "calculateMFC routine to overwrite")
            
            df = df.join(Matrix_Landfilled_noncollected)
            ...
            PG = PG.mul(100/(100-df['mod_EOL_pg2_resell']), axis=0)
            ...
            SUMS1 = (df['mod_EOL_pg1_landfill'] + df['mod_EOL_pg2_resell']+
                     df['mod_EOL_pg3_stored'] + df['mod_EOL_pg4_reMFG']+
                     df['mod_EOL_pg5_recycled'])
            SUMS2 = (df['mod_EOL_pg2_resell']+
                     df['mod_EOL_pg3_stored'] + df['mod_EOL_pg4_reMFG']+
                     df['mod_EOL_pg5_recycled'])
            ...
            # PATH3
            print(PG.size, "SIZE", type(PG))
            print(df['mod_EOL_pg1_landfill'].size, type(df['mod_EOL_pg1_landfill']))
            PG1_landfill = PG.mul(df['mod_EOL_pg1_landfill'].values*0.01)
            df['PG1_landfill'] = list(PG1_landfill.sum())

            # PATH3
            PG3_stored = PG.mul(df['mod_EOL_pg3_stored'].values*0.01)
            df['PG3_stored'] = list(PG3_stored.sum())
            # TODO: Future development of Stored path here.
            
            # PATH4
            PG4_reMFG = PG.mul(df['mod_EOL_pg4_reMFG'].values*0.01)
            ...
            # PATH 5
            PG5_recycled = PG.mul(df['mod_EOL_pg5_recycled'].values*0.01)
