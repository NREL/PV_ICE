...
cwd = os.getcwd() #grabs current working directory
df_installs_raw = pd.read_csv(os.path.join(supportMatfolder,'PVInstalls_World_AllSources.csv'), index_col='Year', skiprows=[1])
...
plt.legend(bbox_to_anchor=(0, 1, 1, 0), loc="lower left", fontsize=12)
...
installs_old_Goetz = pd.DataFrame(installs_old[sources[4]])
...
#Before 2008 = Fraunhofer
mrktshareSi_global_old = pd.DataFrame(df_raw_mrktshr_siVtf.loc[(df_raw_mrktshr_siVtf.index<=2007),refs[2]])
mrktshareSi_global_old.columns = ['Global_MarketShare']
#mrktshare_global_old

#After 2008 = Mints
marketshareSi_global_new = pd.DataFrame(df_raw_mrktshr_siVtf.loc[(df_raw_mrktshr_siVtf.index>2007),refs[1]])
marketshareSi_global_new.columns = ['Global_MarketShare']

mrktshr_global = pd.concat([mrktshareSi_global_old,marketshareSi_global_new])
#mrktshr_global
...
world_si_installs.to_csv(os.path.join(supportMatfolder, 'output_Global_SiPV_installs.csv'))
...
# ### Market Share Weight Countries installs by Silicon
