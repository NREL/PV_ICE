...
testfolder = str(Path().resolve().parent.parent / 'PV_ICE' / 'TEMP' / 'RvL-Energy')
...
for scen in range(len(SFscenarios)):
    ...
    r1.createScenario(name='PVICE', massmodulefile=modulefile) #change name=SFscenarios[scen] if multiple scenarios
    for mat in MATERIALS:
        materialfile = r'..\..\baselines\baseline_material_mass_'+str(mat)+'.csv'
        r1.scenario['PVICE'].addMaterial(mat, massmatfile=materialfile)
...
reliability_baselines['mod_lifetime'] = r1.scenario['PVICE'].dataIn_m['mod_lifetime']
reliability_baselines['mod_reliability_t50'] = r1.scenario['PVICE'].dataIn_m['mod_reliability_t50']
reliability_baselines['mod_reliability_t90'] = r1.scenario['PVICE'].dataIn_m['mod_reliability_t90']
...
pvice_cums = cumRvL_identinstall.filter(like='PVICE')
