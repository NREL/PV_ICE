...
# # Solar Futures ReEDS Scenarios for Lifetime vs Recycling Paper
...
#     Notes:
#    
# Scenario of Interest: Decarb+E (95-by-35+Elec.Adv+DR)
# 
...
testfolder = str(Path().resolve().parent.parent.parent / 'PV_ICE' / 'TEMP' / 'SFs_LifeVSRecycle')
...
# #### Downselect to Solar Future scenario of interest
# 
...
#SFscenarios = [simulationname[0], simulationname[4], simulationname[8]] 'Reference.Mod', '95-by-35.Adv', 
SFscenarios = ['95-by-35_Elec.Adv_DR']
SFscenarios


# #### Add Materials for the simulation
...
for i in range(0, 3):
    ...
    for mat in range(len(MATERIALS)):
        ...
        rr.scenario[SFscenarios[i]].addMaterial(MATERIALS[mat], file=MATERIALBASELINE)
...
rr.plotScenariosComparison('new_Installed_Capacity_[MW]')


# In[69]:


rr.plotScenariosComparison('Installed_Capacity_[W]')
...
plt.plot(UScum.index, UScum['new_Installed_Capacity_[MW]95-by-35_Elec.Adv_DR'], label='Decarb+E')
plt.plot(UScum.index, UScum['new_Installed_Capacity_[MW]95-by-35.Adv'], label='Decarb')
plt.plot(UScum.index, UScum['new_Installed_Capacity_[MW]Reference.Mod'], label='Reference')
plt.ylim(0,1.8e6)
plt.legend()
